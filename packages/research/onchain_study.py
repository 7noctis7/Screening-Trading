...
def _tvl_hist(sym: str) -> dict[str, float]:
    meta = COINS.get(sym, {})
    if "chain" in meta:
        url = _LLAMA_CHAIN_HIST.format(chain=meta["chain"])
        return parse_llama_hist(_get_json(url))
    if "proto" in meta:
        return parse_llama_proto(_get_json(_LLAMA_PROTO.format(slug=meta["proto"])))
    return {}


def coin_series(sym: str, days: int = 365):
    """(rendements, z causal du TVL/MCap) alignés par jour. None si insuffisant."""
    import numpy as np
    ...
    tvl = _tvl_hist(sym)
    common = sorted(set(chart) & set(tvl))
    rows = [(d, chart[d]["price"], chart[d]["mcap"], tvl[d]) for d in common
            if chart[d].get("mcap")]
    if len(rows) < 60:
        return None
    closes = np.array([r[1] for r in rows], float)
    rets = closes[1:] / closes[:-1] - 1.0
    factor = np.array([r[3] / r[2] for r in rows], float)[1:]   # TVL/MCap, aligné rets
    return rets, zscore_causal(factor, 30)


def run_study(symbols: list[str] | None = None, post: int = 5,
              threshold: float = 1.5) -> dict:
    """Event-study cross-actif du facteur TVL/MCap (placebo). Via funding_study."""
