...
GRILLE ?= config/exploration/2026-09-25_quotidien.yaml
explorer:            ## banc PRÉ-ENREGISTRÉ : classe tous les scénarios (DSR, PBO) — période cachée jamais lue
	$(PYTHON) scripts/explorer.py --grille $(GRILLE) $(ARGS)
...
skills-lint:         ## vérifie les CONTRATS de Skills (skills/**/*.skill.yaml) — gate dur
...
x-miroirs:           ## quel miroir RSS de X répond AUJOURD'HUI — rend la ligne QUANT_X_RSS
	$(PYTHON) scripts/x_miroirs.py $(ARGS)
rsshub:              ## RSSHub auto-hébergé (boucle locale) : lit les comptes X — ARGS=statut|arret
	bash scripts/rsshub.sh $(ARGS)
