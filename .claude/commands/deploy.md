...
4. **Committer** sur la branche `claude/screening-trading-platform-me9p11` (jamais `main`), message clair, sans identifiant de modèle.
5. **Pousser** : `git push -u origin claude/screening-trading-platform-me9p11` (retry x4 backoff si erreur réseau).
6. **PR → squash-merge** vers `main` (via les outils GitHub MCP). Ne créer une PR que si demandé/attendu.
7. **Resync** la branche :
   `git fetch origin main -q && git reset --hard origin/main -q && git push -u origin claude/screening-trading-platform-me9p11 --force`.
