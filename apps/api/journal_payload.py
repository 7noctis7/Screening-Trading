...
def construire(journal, prix: dict[str, float], positions: dict[str, float]) -> dict:
    ...
    """
    from packages.execution.perimetre_journal import origine, pris_par_le_robot
    from packages.research.exec_costs import measured_slippage

    tous = journal.all()
    # LES DEUX PÉRIMÈTRES, CÔTE À CÔTE (réconciliation du 21/09).
    #
    # `rows` reste le périmètre ROBOT : tous les taux de réussite, espérances et
    # matérialités de la page s'y rapportent, et les élargir en silence aurait changé la
    # signification de chiffres déjà lus sans que rien ne le dise.
    #
    # `rows_tous` porte l'historique ENTIER du compte, chaque ligne étiquetée par son
    # ORIGINE. Les deux questions — « qu'a fait mon robot » et « qu'a fait ce compte » —
    # sont légitimes, elles n'ont simplement pas la même réponse ; leurs totaux respectifs
    # sont dans `stats.perimetre`, et l'écart entre eux est le sujet.
    lignes = [{**_ligne(t), "origine": origine(t.id)} for t in tous]
    rows = [r for r, t in zip(lignes, tous, strict=True) if pris_par_le_robot(t.id)]
    ...
    return {"available": True, "rows": closed, "ouverts": ouverts,
            "rows_tous": [r for r in lignes if r["exit_ts"]],
            "ouverts_tous": [r for r in lignes if not r["exit_ts"]],
