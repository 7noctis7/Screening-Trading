  ...
  async function ask(value?: string) {
    ...
    try {
      const r = await fetch(`${BASE}/api/ai/chat`, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...enTetesIA() },
  ...
  return (
    <>
      <button
        ...
        aria-label="Interroger Quant Terminal"
      ...
      {open && (
        ...
        >
          <section
            className="h-full w-full max-w-[480px] border-l border-border flex flex-col"
            style={{ background: "var(--bg)" }}
            role="dialog"
            aria-label="Copilote quantitatif"
          >
            <header className="p-4 border-b border-border flex items-start justify-between gap-3">
              <div>
                <h2 className="font-semibold">Copilote Quant Terminal</h2>
                <p className="text-xs text-muted mt-1">
                  Lecture seule · scope {scope} · aucune exécution
                </p>
              </div>
            ...
            <div className="p-3 border-b border-border flex flex-wrap gap-2">
              {(SUGGESTIONS[scope] ?? SUGGESTIONS.overview).map((s) => (
                <button
                  key={s}
                  onClick={() => ask(s)}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {!messages.length && (
                <p className="text-sm text-muted">
                  Pose une question sur la page courante. Les réponses quantitatives sont
                  contrôlées et citées.
                </p>
              )}
              {messages.map((m, i) => (
                <div
                  ...
                  style={{
                    background:
                      m.role === "user" ? "var(--surface2)" : "var(--surface)",
                  }}
                >
              ...
              {loading && (
                <p className="text-sm text-muted">Analyse des sources autorisées…</p>
              )}
              <div ref={end} />
            </div>
            <footer className="p-4 border-t border-border">
              <label className="flex gap-2 text-xs text-muted mb-2">
                <input
                  type="checkbox"
                  checked={details}
                  onChange={(e) => setDetails(e.target.checked)}
                />{" "}
                Inclure les positions détaillées dans le contexte cloud
              </label>
              <div className="flex gap-2">
                <textarea
                  ...
                  className="flex-1 rounded-lg border border-border p-2 text-sm text-fg"
                  style={{ background: "var(--surface)" }}
                />
                <button
                  onClick={() => ask()}
                  disabled={loading || question.trim().length < 3}
                  className="px-3 rounded-lg border border-border disabled:opacity-40"
                >
                  Envoyer
                </button>
              </div>
              ...
              >
                Effacer la conversation locale
              </button>
            </footer>
          </section>
        </div>
