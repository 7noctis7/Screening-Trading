...
from packages.backtest.preset_helpers import adaptive_cap, cap_weights
...
def test_cap_weights_noop_sous_le_plafond():
    ...
    assert np.allclose(cap_weights(w.copy(), 0.25), w)


def test_adaptive_cap_resserre_en_stress_corr():
    calm = adaptive_cap(_cov(10, 0.20), 0.10, corr_tighten=True)
    stress = adaptive_cap(_cov(10, 0.80), 0.10, corr_tighten=True)
...
def test_adaptive_cap_off_par_flag():
    assert adaptive_cap(_cov(10, 0.9), 0.10, corr_tighten=False) == 0.10
