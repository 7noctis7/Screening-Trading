...
différents et exclusifs : lever un plafond de jetons, désactiver un mode « raisonnement »,
...
def test_une_reponse_TRONQUEE_nomme_le_plafond_de_jetons():
    """Le cas le plus traître : la réponse est coupée au milieu du JSON, donc illisible —
    et rien dans le texte reçu ne dit que c'est une troncature."""
...
def test_le_remede_propose_un_plafond_PLUS_HAUT_que_celui_en_vigueur():
    """Répéter le plafond courant ferait croire à un mur alors qu'il reste un réglage."""
...
def test_un_modele_a_RAISONNEMENT_est_reconnu_comme_tel():
    ...
    m = pourquoi_illisible("", {"reasoning_content": "Analysons ce titre…"}, "stop", 400)
...
def test_un_vrai_JSON_malforme_MONTRE_ce_qui_a_ete_recu():
    """Le seul cas où le remède est de corriger l'invite — encore faut-il voir la réponse."""
...
def test_le_repli_PORTE_le_motif_du_pilote_jusqu_au_rapport():
    """Sans cela, `nlp-check` affiche « REPONSE_ILLISIBLE » et l'utilisateur reste devant
    un mur : le pilote SAVAIT pourquoi, l'information était simplement perdue."""
...
def test_le_moteur_transmet_le_plafond_de_la_config_au_pilote(monkeypatch):
    ...
    m.pilote()
    # La config ENTIÈRE est passée : plus aucun champ à recopier, donc plus aucun à perdre.
...
def test_nlp_check_honore_TOUS_les_reglages_de_l_environnement(monkeypatch):
    ...
    `max_jetons` à l'ajout du champ. `QUANT_NLP_MAX_JETONS=1200` restait sans effet, et le
...
def test_AUCUN_appelant_ne_reconstruit_un_pilote_champ_par_champ():
    ...
    Trois appelants sur six avaient recopié `choisir(cfg.modele, cfg.pilote, cfg.base)` et
    ...
    assert not coupables, ("ces appelants reconstruisent un pilote à la main et perdront "
                           f"le prochain champ ajouté : {coupables}")
