...
def test_la_table_lit_les_lignes_DEJA_filtrees_par_l_api():
    ...
    assert "data.rows_tous : data.rows" in PAGE
    assert "data.ouverts_tous : data.ouverts" in PAGE
    assert "data.rows.filter" not in PAGE
    assert "data.rows_tous.filter" not in PAGE
...
def test_aucun_filtre_de_STATUT_ne_subsiste():
    ...
    # La version d'origine interdisait TOUT `useState` : elle visait juste et disait
    # trop. La bascule de PÉRIMÈTRE (robot / compte entier) en a légitimement besoin, et
    # elle ne peut pas vider la table — les deux listes viennent de l'API. C'est le
    # filtre de STATUT qui est proscrit, pas l'état.
    assert '"status"' not in PAGE, "la colonne Statut ne dirait plus qu'une chose"
    for interdit in ("setStatut", "setStatus", "filtreStatut"):
        assert interdit not in PAGE, "aucun sélecteur ouvert/fermé ne doit revenir"
